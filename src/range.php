 ...
 * ```
 *
 * @param int|float $start Starting value for the sequence.
 * @param int|float $end Ending value for the sequence (inclusive). The last element may equal
 *                       or be less than/greater than this value depending on step alignment.
 * @param int|float $step Increment or decrement value for each step in the sequence. Must be
 *                        non-zero and directionally consistent with start/end (positive for
 *                        ascending, negative for descending). Default: 1.
 *
 * @return array<int, int|float> An indexed array containing the generated numeric sequence.
 *
...
{
    // Use == to catch both 0 and 0.0
    throw_if($step == 0, ZeroStepException::create());
