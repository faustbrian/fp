...
use Tests\Exceptions\AlwaysFailException;
use Tests\Exceptions\FinalErrorException;
use Tests\Exceptions\FirstAttemptException;
use Tests\Exceptions\RetryNeededException;
use Tests\Exceptions\SecondAttemptException;
use Tests\Exceptions\SimulatedAttemptException;
    ...
    describe('Happy Paths', function (): void {
        ...
        test('retries and succeeds after initial failures', function (): void {
            ...
            $callable = function () use (&$attempts): string {
                ...
                throw_if($attempts < 3, SimulatedAttemptException::forAttempt($attempts));
    ...
    describe('Sad Paths', function (): void {
        test('throws last exception after exhausting all retries', function (): void {
            ...
            $callable = function () use (&$attempts): void {
                ...
                throw SimulatedAttemptException::forAttempt($attempts);
            };

            $retrier = retry(3);

            expect(fn () => $retrier($callable))
                ->toThrow(SimulatedAttemptException::class, 'Attempt 3 failed');
            expect($attempts)->toBe(3);
        });
        ...
        test('throws last exception with different exception types', function (): void {
            ...
            $callable = function () use (&$attempts): void {
                ...
                throw_if($attempts === 1, FirstAttemptException::create());

                throw_if($attempts === 2, SecondAttemptException::create());

                throw FinalErrorException::create();
            };

            $retrier = retry(3);

            expect(fn () => $retrier($callable))
                ->toThrow(FinalErrorException::class, 'Final error');
            expect($attempts)->toBe(3);
        });
        ...
        test('fails immediately with single attempt allowed', function (): void {
            ...
            $callable = function () use (&$attempts): void {
                ...
                throw SimulatedFailureException::create();
            };

            $retrier = retry(1);

            expect(fn () => $retrier($callable))
                ->toThrow(SimulatedFailureException::class, 'Failed');
            expect($attempts)->toBe(1);
        });
    });
    ...
    describe('Edge Cases', function (): void {
        ...
        test('works with callable returning arrays', function (): void {
            ...
            $callable = function () use (&$attempts): array {
                ...
                throw_if($attempts < 2, RetryNeededException::create());
    ...
    describe('Backoff Functionality', function (): void {
        test('retries without delay when backoff is null', function (): void {
            ...
            $callable = function () use (&$attempts): string {
                ...
                throw_if($attempts < 3, RetryNeededException::create());
        ...
        test('backoff receives correct attempt numbers (1-indexed)', function (): void {
            ...
            $callable = function () use (&$attempts): void {
                ...
                throw AlwaysFailException::create();
            };
        ...
        test('exponential backoff example', function (): void {
            ...
            $callable = function () use (&$attempts): string {
                ...
                throw_if($attempts < 4, RetryNeededException::create());
